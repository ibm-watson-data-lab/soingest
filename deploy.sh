...
if [ -z ${BOT_URL+x} ]; then echo 'Error: Environment variable BOT_URL is undefined.'; invocation_error=1; fi
...
./Bluemix_CLI/bin/bluemix wsk package update $WSK_PACKAGE -p cloudantURL $CLOUDANT_URL -p dbname $QUESTIONS_DB -p slackURL $SLACK_URL -p apikey $STACKOVERFLOW_API_KEY -p botURL $BOT_URL
