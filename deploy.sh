...
bx wsk package update $WSK_PACKAGE -p cloudantURL $CLOUDANT_URL -p dbname $QUESTIONS_DB -p slackURL $SLACK_URL -p apikey $STACKOVERFLOW_API_KEY

bx wsk action update $WSK_PACKAGE/collector collector/collector.js 

bx wsk action update --kind nodejs:6 $WSK_PACKAGE/invoker invoker/invoker.js

# create a sequence of both actions
bx wsk action update $WSK_PACKAGE/socron --sequence $WSK_PACKAGE/collector,$WSK_PACKAGE/invoker


# actions for store and notify, and a sequence for those

bx wsk action update $WSK_PACKAGE/storer storer/storer.js
bx wsk action update $WSK_PACKAGE/notifier notifier/notifier.js

bx wsk action update $WSK_PACKAGE/qhandler --sequence $WSK_PACKAGE/storer,$WSK_PACKAGE/notifier
