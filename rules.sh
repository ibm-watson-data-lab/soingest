...
./wsk trigger create so_spark --feed /whisk.system/alarms/alarm --param cron "7,27,47 * * * *" --param trigger_payload "{\"tags\": [\"apache-spark\", \"ibm-bluemix\"], \"search_type\": \"and\"}"
./wsk rule update so_sparkrule so_spark stackoverflow/socron
./wsk rule enable so_sparkrule

# juptyer search
./wsk trigger delete so_jupyter
./wsk trigger create so_jupyter --feed /whisk.system/alarms/alarm --param cron "7,27,47 * * * *" --param trigger_payload "{\"tags\": [\"jupyter-notebooks\", \"ibm-bluemix\"], \"search_type\": \"and\"}"
